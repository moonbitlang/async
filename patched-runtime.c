...
MOONBIT_EXPORT void moonbit_panic(void) {
  ...
  if (!SymInitialize(curr_process, NULL, FALSE))
    goto error;
  int n = CaptureStackBackTrace(
    0,
    sizeof(stack_trace) / sizeof(void*),
    stack_trace, NULL
  );

  char symbol_info_buffer[sizeof(SYMBOL_INFO) + 255 * sizeof(TCHAR)];
  ...
  for (int i = 0; i < n; ++i) {
    if (!SymFromAddr(curr_process, (DWORD64)stack_trace[i], 0, &symbol_info))
      goto error;
    DWORD offset;
    if (!SymGetLineFromAddr64(curr_process, (DWORD64)stack_trace[i], &offset, &line_info))
      goto error;
    fprintf(
      stderr,
      "%s (%s:%d+%d)\n",
      symbol_info->Name,
      line_info->FileName,
      line_info->LineNumber, offset
    );
  }
error:
  fflush(stderr);
#else
  int n = backtrace(stack_trace, sizeof(stack_trace) / sizeof(void*));
  char **symbols = backtrace_symbols(stack_trace, n);
  for (int i = 0; i < n; ++i)
